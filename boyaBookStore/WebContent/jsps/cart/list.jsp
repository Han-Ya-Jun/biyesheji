	...
	$(".jian").click(function() {
		...
		if(quantity == 1) {
			if(confirm("您是否真要删除该条目？")) {
				location = "/batchDelete.action?cartItemIds=" + id;
			}
		} else {
...
function sendUpdateQuantity(id, quantity) {
	$.ajax({
		async:false,
		cache:false,
		url:"/boyaBookStore/updateQuantity.action",
...
function batchDelete() {
	...
	window.location.href = "/boyaBookStore/batchaDelete.action?cartItemIds=" + cartItemIdArray;
}
...
<c:forEach items="${cartItemList }" var="cartItem">
	<tr align="center">
		...
		<td align="left" width="70px">
			<a class="linkImage" href="<c:url value='/loadbook.action?bid=${cartItem.book.bid }'/>"><img border="0" width="54" align="top" src="<c:url value='/${cartItem.book.imageB }'/>"/></a>
		</td>
		<td align="left" width="400px">
		    <a href="<c:url value='/loadbook.action?bid=${cartItem.book.bid }'/>"><span>${cartItem.book.bname }</span></a>
		</td>
		...
		<td>
			<a href="<c:url value='/batchaDelete.action?cartItemIds=${cartItem.cartitemid }'/>">删除</a>
		</td>
	</tr>
</c:forEach>
